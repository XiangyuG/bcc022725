...
#define SVCIP 0x0A6B47C0  // 10.107.71.192 (hex representation)
#define NEW_DST_IP 0x0A00016E  // 10.0.1.110 (hex representation)
#define NEW_DST_IP2 0x0A00016D  // 10.0.1.109
...
// BPF_TABLE("lru_hash", struct flow_key, struct flow_val, nat_map, 65536);
       ...
       if (dst_ip == SVCIP) {
            ...
            if (ip.protocol == IPPROTO_TCP) {
                ...
                if (tcp.syn && !tcp.ack) {
                    ...
                    struct flow_val new_val = { .pod_ip = new_dst_ip, .svc_ip = bpf_htonl(SVCIP), .last_seen_ns = bpf_ktime_get_ns() };
                    nat_map.update(&key, &new_val);
                } else {
                    u64 now = bpf_ktime_get_ns();

                    struct flow_val *val = nat_map.lookup(&key);
                    if (!val) {
                        u32 rand_val = bpf_get_prandom_u32() % 10;
                        if (rand_val < 5) {
                            new_dst_ip = bpf_htonl(NEW_DST_IP2);
           ...
           // bpf_trace_printk("Failed to Come here %d\\n", ifindex);
           return TC_ACT_OK;
       }   
   }
   return TC_ACT_OK;
...
int redirect_pod_to_service(struct __sk_buff *skb) {
   ...
   if (!rev_val) {
       // bpf_trace_printk("Come here\\n");
       return TC_ACT_OK;
   } else {
        new_src_ip = bpf_ntohl(rev_val->client_ip); // From pod IP to svc IP
        // Store the updated destination IP in the packet   
...
interface1 = "lxcb4db85b61ffe"
interface2 = "lxc1b773a8020e2"
