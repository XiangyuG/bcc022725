...
BPF_ARRAY(latency_array, u64, 20);
BPF_HASH(key_array_idx, struct ip_port_key, u32);
BPF_ARRAY(avail_array_idx, u32, 1);
   ...
   if (src_ip == frontendPodIP) {   
       if (dst_ip == svcip) {
           ...
           if (exist_dst_ip) {
              ...
              // Get array ID
              // BPF_ARRAY(avail_array_idx, u32, 1);
              u32 avail_array_idx_key = 0;
              u32 *array_idx_v = avail_array_idx.lookup(&avail_array_idx_key);
              if (array_idx_v) {
                key_array_idx.update(&key, array_idx_v);
                *array_idx_v = ((*array_idx_v) + 1) % 100;
                avail_array_idx.update(&avail_array_idx_key, array_idx_v);
              }
           }
...
int redirect_pod_to_service(struct __sk_buff *skb) {
   ...
   if (dst_ip == frontendPodIP) {   
       if (src_ip == serverPod0IP || src_ip == serverPod1IP || src_ip == serverPod2IP) {
           ...
           key.ip = dst_ip; 
           u16 dst_port;
            if (parse_l4(skb, &ip, &dst_port, &key.port) < 0) {
                    bpf_trace_printk("parse_l4(skb, &ip, &dst_port, &key.port) < 0\\n");
                    return TC_ACT_OK;
            }
            bpf_trace_printk("port ID is %d\\n", key.port);
            // TODO: update latency
            ...
            u64 *pre_time = time_dnat.lookup(&key);
            if (pre_time) {
                u64 latency_value = now - (*pre_time);
                u32 *update_array_idx = key_array_idx.lookup(&key);
                if (update_array_idx) {
                    bpf_trace_printk("Update latency array\\n");
                    latency_array.update(update_array_idx, &latency_value);
                }
            }
...
try:
   ...
   avail_array_idx = b["avail_array_idx"]
   avail_array_idx[ctypes.c_uint(0)] = ctypes.c_uint(0)
...
#    b.trace_print()
   while True:
        ...
        latency_array_map = b["latency_array"]

        # Iterate through all entries in the map
        print("Current entries in time_stamp map:")
        for key, value in latency_array_map.items():
            key_value = key.value
            out_value = value.value  # value is a ctypes type, so use `.value`
            if out_value > 0:
                print(f"curl request ID: {key_value}, latency: {out_value / 1000000000.0} seconds")
        time.sleep(10)
finally:
