...
BPF_ARRAY(multiple, u64, 1);
BPF_ARRAY(total, u64, 1);
BPF_ARRAY(success, u64, 1);
...
BPF_HASH(decision, struct ip_port_key, u32);  // decision
...
int drop_packet(struct xdp_md *ctx) {
    ...
    if (eth->h_proto == htons(ETH_P_IP)) {  // If it's an IPv4 packet
        ...
        if (src_ip == htonl(0x0AF40004) && (dst_ip == serverPod0IP || dst_ip == serverPod1IP || dst_ip == serverPod2IP)) {
            struct ip_port_key key = { .ip = 0xC0A80101, .port = 12345 }; 
            ...
            u32 total_key_idx = 0;
            u64 *total_v = total.lookup(&total_key_idx);
            u32 multiple_key_idx = 0;
            u64 *multiple_v = multiple.lookup(&multiple_key_idx);
            u32 success_key_idx = 0;
            u64 *success_v = success.lookup(&success_key_idx);
            ...
            if (pre_time == NULL || (now - (*pre_time)) > expiration) {
                if (pre_time) {
                    bpf_trace_printk("pre_time = %llu\\n", (*pre_time));
                }
                time_stamp.update(&key, &now);
                if (total_v && multiple_v && success_v) {
                    bpf_trace_printk("total_v = %d\\n", (*total_v));
                    bpf_trace_printk("multiple_v = %d\\n", (*multiple_v));
                    bpf_trace_printk("success_v = %d\\n", (*success_v));
                    bpf_trace_printk("rand_val = %d\\n", rand_val);
                    u64 max_v = 0;
                    if ((*total_v) - (*multiple_v) * (*success_v) > 0) {
                        max_v = (*total_v) - (*multiple_v) * (*success_v);
                    }
                    u32 zero = 0; // zero means no drop
                    u32 one = 1; // one means drop
                    
                    bpf_trace_printk("((*total_v) + 100 - max_v) * 100 = %d\\n", ((*total_v) + 100 - max_v) * 100);
                    if (((*total_v) + 100 - max_v) * 100 > rand_val * (*total_v)) {
                        (*total_v) = (*total_v) + 100;
                        total.update(&total_key_idx, total_v);
                        bpf_trace_printk("Decision drop packet:\\n");
                        decision.update(&key, &one); // one means drop;
                        return XDP_DROP;  // Drop the packet
                    }
                    (*total_v) = (*total_v) + 100;
                    total.update(&total_key_idx, total_v);
                    decision.update(&key, &zero); // zero means pass;
                    bpf_trace_printk("Decision pass packet:\\n");
                    return XDP_PASS;
                }
                return XDP_PASS;
            } else {
                ...
                if (signal) {
                    if ((*signal) == 0) {
                        bpf_trace_printk("Continue pass packet:\\n");
                        return XDP_PASS;
                    } else {
                        bpf_trace_printk("Continue dropping packet:\\n");
                        return XDP_DROP;  // Drop the packet
                    }
                }
                return XDP_PASS;
...
b = BPF(text=bpf_code)
multiple = b["multiple"]
multiple[ctypes.c_uint(0)] = ctypes.c_uint(95)
success = b["success"]
success[ctypes.c_uint(0)] = ctypes.c_uint(0)
total = b["total"]
total[ctypes.c_uint(0)] = ctypes.c_uint(0)
